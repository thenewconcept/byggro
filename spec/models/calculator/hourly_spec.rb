...
RSpec.describe Calculator::Hourly, type: :model do
  describe 'a project 10000 budget and 500 hourly rate' do
    ...
    describe 'with an employee with 200 salary' do
      ...
      let(:calc)    { Calculator::Hourly.for(project, employee) }
      ...
      describe '#bonus_base(salary)' do
        it 'calculates the bonusbase for a given salary and INDEX' do
          ...
          stub_const('Calculator::Hourly::BONUS_INDEX', 280)
          expect(calc.bonus_base(100)).to eq(100)
    ...
    describe 'with an employee with 100 salary' do
      ...
      it '#bonus_base' do
        ...
        stub_const('Calculator::Hourly::BONUS_INDEX', 150)
    ...
    describe 'no bonus' do
      it 'returns 0 and not negative values' do
        ...
        nobonus = Calculator::Hourly.for(overdrawn, jill)
    ...
    describe '#bonus' do
      ...
      it 'returns total bonus for reportee' do
        ...
        expect(Calculator::Hourly.for(project, john).bonus).to eq(125)
        expect(Calculator::Hourly.for(project, jim).bonus).to eq(250)
      end

      it 'cannot be negative' do
        stub_const('Calculator::Hourly::BONUS_INDEX', 300)
        create(:report, time_in_hours: 40, reportable: checklist, reportee: john)
        expect(Calculator::Hourly.for(project, john).bonus).to eq(0)
      end
    end
    ...
    describe '#total' do
      ...
      it 'returns total bonus for project' do
        # 5h x 0 = 0.
        expect(Calculator::Hourly.for(project, jill).total).to eq(0)

        # (S: 4h x 200 = 800) + (B: 100 * 0.20 * 4 = 80) = 880 kr
        expect(Calculator::Hourly.for(project, john).total).to eq(880)

        # (S: 4h x 100 = 400) + (B: 100 * 0.20 * 4 = 80) = 480 kr
        expect(Calculator::Hourly.for(project, jim).total).to eq(480)

        # Jake is contractor, so fast fee.
        expect(Calculator::Hourly.for(project, jake).total).to eq(2000)
      end

      it 'returns the total salaries for the project' do
        expect(Calculator::Hourly.for(project, nil).total).to eq(3360)
      end
    end
  end
end
