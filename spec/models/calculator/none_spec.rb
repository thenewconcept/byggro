...
RSpec.describe Calculator::None, type: :model do
  describe 'a project 10000 budget and 500 hourly rate' do
    ...
    describe 'with an employee with 200 salary' do
      ...
      it '#bonus_percent' do
        expect(Calculator::None.for(project, nil).bonus_percent).to eq(0)
      end
      ...
      describe '#salary' do
        it 'pays sum of time reported' do
          ...
          expect(Calculator::None.for(project, employee).salary).to eq(3000)
          expect(Calculator::None.for(project, contractor).salary).to eq(6000)
        end
      end
    end
  end
