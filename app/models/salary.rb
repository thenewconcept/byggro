class Salary
  ...
  def payable_bonus_projects
    @payable_bonus_projects ||= Project.where(bonus: 'fixed', completed_on: @from..@to).or(Project.where(bonus: 'hourly', completed_on: @from..@to)).status_completed
  end

  def sales_bonus_projects
    @sales_bonus_projects ||= Project.where(payed_on: (@from - 2.months)..(@to - 2.months)).where.not(seller: [false, nil, ''])
  end
  ...
  def payable_hourly_reports
    @payable_hourly_reports ||= Report
      ...
      .or( Report.payable.where(reportable: Checklist.where(project: payable_hourly_projects), reportee_type: ['Employee'], date: @from..@to) )
      .or( Report.payable.where(reportee_type: 'Contractor', date: @from..@to) )
  end
