class Report < ApplicationRecord
  ...
  scope :by_reportable, -> (reportable) { where(reportable: reportable).order(id: :asc, date: :asc) }
  scope :by_project,   -> (project) { where(reportable: project).or(self.where(reportable: project.checklists)).order(id: :asc, date: :asc) }
  scope :by_workers,   -> (reportable) { by_reportable(reportable).where.not(reportee_type: 'Contractor').order(id: :asc, date: :asc) }
  ...
  def checklist_id
    reportable_type == 'Checklist' ? reportable_id : nil
  end
